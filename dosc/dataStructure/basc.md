...
###### 5. 排序
   ...
   - 算法原理
     + O(n2): 
       1. 冒泡排序: 对数据进行两次嵌套遍历,第一次控制总遍历次数,第二层比较相邻元素,每次都按条件对元素进行交换. 
       ...
       4. 希尔排序: (最好情况O(Nlog2N),最坏情况O(N2)) 对数组设置一个间隔,按间隔组成多个子数组并进行直接插入排序,渐渐缩小间隔直到间隔为1,变成完全的直接插入排序.
       - 对比: 
         1. 冒泡和插入排序是稳定的排序算法,因为两个相同元素不会再次交换位置,而选择排序若存在同样大小的元素会导致他们的位置发生变化
         2. 效率对比: 希尔(不稳定) > 插入(稳定) > 选择(不稳定) > 冒泡(稳定).
         3. 插入排序每次遍历只需要一次赋值操作, 选择排序需要一次赋值,但比较次数更多, 冒泡排序每次遍历都要进行三个赋值操作.
         4. 希尔排序是插入排序的优化升级版,插入排序在大多数元素有序或元素比较少的情况下性能最优,而希尔排序刚好符合这两个条件,子数组两两比较交换符合少元素原则,大间隔两两比较到最后可保证元素基本有序.
     + O(nlogn):
       1. 归并排序: 本质上是将原始数组二分,先从左边开始递归直到找到最小间隔[即左右两组的元素数都为1],跳出循环.然后对该组元素进行排序,再从栈内依次取出元素下标进行判断,处理数据.直到元素全部对比完成.
       2. 快速排序: 
       - 对比:
         1. 效率对比: 归并排序(不稳定) > 快排(稳定)
         2. 复杂度: 归并排序(时间复杂度恒定nlogn,空间复杂度n), 快排(时间复杂度平均nlogn,最坏n2)
#### 内存相关(零碎知识点,后期整理)
> 写链表时关于赋值出现的疑问,特此记录
  - 数据传递
    + 值传递
